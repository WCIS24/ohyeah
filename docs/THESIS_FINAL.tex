...
金融领域的专业问题常涉及最新的市场数据、监管条款与财报细节，具有信息密度高、语义压缩强的特点。传统大型语言模型在金融问答中容易产生事实性错误或因知识滞后而偏离证据。为提高可靠性，检索增强生成（RAG）通过“检索—生成”机制使回答基于真实文档，但现有多数金融 RAG 仍以\textbf{单步检索}为主，面对复杂金融查询时表现不足。

复杂查询通常具备以下特征：
\begin{itemize}
\item 信息分散：所需证据散落于多个段落或多份文档；
\item 结构化需求：涉及同比、差值、占比等显式计算；
\item 语义压缩：包含金融缩写与术语（如“MS”“YoY”），需进行实体消歧与扩写。
\end{itemize}

FinDER 数据集由金融领域专家构建，包含 5,703 个真实金融问答三元组（查询—证据—答案），其中大量查询简短含糊且依赖跨证据推理。该数据集凸显了“多步检索 + 结构化推理”的必要性。

\textbf{本文思路}：引入规则驱动的多步检索推理机制，使系统能像分析员一样逐步补全证据链；同时引入显式计算模块，降低数值类问题的算术错误。系统在工程实现上坚持开源、可复现与可审计，确保实验结果可追踪与可复验。
...
本文系统由四个核心模块构成：\textbf{查询理解}、\textbf{多步检索推理}、\textbf{证据整合与计算}、\textbf{答案生成}。系统采用流水线式数据流，模块间接口明确，便于复现与扩展。

\textbf{图1 多步检索循环流程（示意）}
...
\begin{itemize}
\item 规则与词典扩展（如将“YOY”扩展为“同比”）；
\item 金融领域 NER 与实体链接；
\item 轻量语义解析（不开启外部 API）。
\end{itemize}
...
\begin{itemize}
\item \textbf{Gap Detector}：判断是否缺失关键年份或比较对象；
\item \textbf{Gate}：若 $\text{gap\_conf} < \tau$，终止后续检索；
\item \textbf{Merge Strategy}：跨步候选去重并按 maxscore 或 step1	extunderscore first 排序；
\item \textbf{Stop Criteria}：达到 $T$ 或连续无新增证据时停止。
\end{itemize}

关键超参数定义：
\begin{itemize}
\item 检索轮数 $T$（max\_steps）
\item 每轮检索 top-$k$（top\_k\_each\_step）
\item 最终截断 top-$k_f$（top\_k\_final）
\item 门控阈值 $\tau$（min\_gap\_conf）
\end{itemize}

\textbf{Step6 最优配置}：$T=2$，top\_k\_each\_step=10，merge=maxscore，novelty\_threshold=0.0，stop\_no\_new\_steps=1，$\tau=0.3$。
...
\textbf{Step6 最优门控}：min\_conf=0.2，allow\_task\_types=[]（当前版本以稳定性为先）。
...
\section{子集定义}
\begin{itemize}
\item \textbf{complex\_dev}：满足任一条件即进入子集：多证据、查询含 ≥2 年份、含比较/变化关键词、或含数值+年份组合。
\item \textbf{numeric\_dev}：查询或答案含数字/百分号/同比/差值/倍数关键词。
\end{itemize}

\section{评价指标与口径}
\begin{itemize}
\item 检索指标：Recall@k、MRR@k、evidence\_hit@k
\item QA 指标：EM/F1（用于对照）
\item 数值指标：Numeric-EM、RelErr、Coverage
\item 不确定匹配比例：当证据缺少 doc\_id/evidence\_id 时使用文本匹配，并记录比例。
\end{itemize}

\section{关键参数}
\begin{itemize}
\item 检索器：BM25 + Dense + Hybrid（alpha=0.5）
\item 多步检索（best）：max\_steps=2，top\_k\_each\_step=10，merge=maxscore
\item 计算器门控（best）：min\_conf=0.2，allow\_task\_types=[]
\end{itemize}
...
\textbf{主结果表见}：`docs/TABLE_MAIN.md`

关键对照（complex dev）：
\begin{itemize}
\item baseline(post-ft) vs best multistep：
...
\begin{itemize}
\item pre-ft baseline → post-ft baseline：Recall@10 0.3246 → 0.3772（+0.0526）
\end{itemize}

\section{2) 数值题表现（numeric dev）}

\textbf{数值表见}：`docs/TABLE_NUMERIC.md`

关键对照（numeric dev）：
\begin{itemize}
\item baseline(post-ft) vs best calc gate：
...
\begin{itemize}
\item 消融结果见 `docs/TABLE_ABLATION.md`
\item 典型复杂查询案例见附录 B（3 个案例，包含多步检索每步 top-3 证据与 stop 原因）
\end{itemize}
...
\section{1. 失败类型概览（Step6）}
\begin{itemize}
\item \textbf{complex dev}：主要集中在 `no_gap` 与 `max_steps`，说明部分查询在第一步已覆盖核心证据，但 refine 能力仍有限；
\item \textbf{numeric dev}：由于计算器门控关闭（allow_task_types=[]），多数样本回退到 baseline，表现为 fallback 占比高。
\end{itemize}

\section{2. 失败原因分析}
1) \textbf{简称歧义与实体对齐不足}：金融缩写可能指向多家公司，导致检索命中无关证据。  
2) \textbf{检索漂移}：refined query 若过于相似或偏离目标，会重复检索或引入噪声。  
3) \textbf{证据冲突与口径不一}：不同段落可能存在统计口径差异（如合并口径与单体口径），需要更强的单位/实体对齐策略。  
4) \textbf{计算器保守门控}：为避免算错，门控阈值偏保守，导致覆盖率受限。  
...
\section{3) 未来工作}
\begin{itemize}
\item 引入更强的检索器与领域适配预训练；
\item 提升实体消歧与 query rewriting 的准确性；
\item 扩展计算器任务类型与多尺度单位转换；
\item 结合轻量 agent 机制增强多步检索的决策质量。
\end{itemize}
...
\begin{itemize}
\item 检索器微调显著提升整体检索表现（full dev Recall@10: 0.3246 → 0.3772）；
\item 多步检索在复杂子集上保持不退化，MRR 略有提升；
\item 计算器门控避免数值指标回退，为后续优化奠定稳定基线。
\end{itemize}
...
\item 分析：该问题包含对比关系与年份信息，多步检索识别到 gap，但 refined query 与原查询高度相似，导致新增证据有限。
\end{itemize}
...
\item gap/stop：MISSING_ENTITY / MAX_STEPS，final_topk_size=10
\item 分析：问题涉及“出售现金流入 vs 并购现金流出”的对比，多步检索能够维持证据覆盖但未显著扩展证据范围。
\end{itemize}
...
\item 分析：该类问题实体明确、语义集中，单步检索即可覆盖核心证据，多步检索不引入额外噪声。
\end{itemize}
